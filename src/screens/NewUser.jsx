...
import { loadAllOneDayUser } from '../redux/actions/userAction';
  
  const NewUser = () => {
    const navigation = useNavigation();
    const dispatch = useDispatch();
    const {accesstoken} = useSelector(state => state.user);
    const {loadingAllOneDayUser, allonedayusers} = useSelector(state => state.user);
    const [filteredData, setFilteredData] = useState([]);
  
    const handleSearch = text => {
      const filtered = allonedayusers.filter(item =>
    ...
    useEffect(() => {
      setFilteredData(allonedayusers); // Update filteredData whenever locations change
    }, [allonedayusers]);
      ...
      <SafeAreaView style={{flex: 1}}>
        ...
        <View
          ...
          <View
            ...
            <GradientText style={styles.textStyle}>New Users</GradientText>
